...
REVIEW_RULES: List[Dict[str, object]] = [
    ...
    },
    {
        ...
        "bullets": [
            ...
            "MANDATORY DATASHEET LOOKUP: If you see a specific component part number in the BOM (e.g., BZX84C5V1, LM358, 2N2222, IRF540N), you MUST call `query_datasheet` BEFORE making ANY claim about that component's parameters (Zener voltage, gain-bandwidth, breakdown ratings, etc.). Do NOT rely on internal memory for part-specific parameters — memory is unreliable and leads to fatal hallucinations.",
            ...
            "You have a maximum of 3 tool calls total. Use them wisely — do not repeat the same query.",
        ],
    },
    {
        "id": "grounding",
        ...
        "bullets": [
            'IRON LAW: You MUST base your ENTIRE voltage, gain, frequency, and transient analysis STRICTLY on the numerical values in `dc_node_voltages`, `ac_metrics`, and `transient_metrics` from the JSON context. These are computed by a deterministic MNA solver — they are ground truth.',
